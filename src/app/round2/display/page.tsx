  ...
  return (
    ...
    >
      ...
      <div className="mb-8 flex items-end justify-between border-b border-[#C8A951]/20 pb-5">
        <div>
          <p className="font-mono text-xs tracking-[0.4em] text-[#C8A951]/70 lg:text-sm">
            M.E.S. ENGLISH MEDIUM SCHOOL
          </p>
          <h1 className="mt-1 text-4xl font-bold tracking-tight text-[#F7F2E7] lg:text-6xl">
            Round 2
          </h1>
        </div>
        ...
        <div className="flex items-end gap-8 lg:gap-14">
          <div className="text-right">
            <p className="font-mono text-[10px] tracking-[0.3em] text-[#F7F2E7]/40 lg:text-xs">
              QUESTION
            </p>
            <p className="font-mono text-4xl font-bold tabular-nums text-[#C8A951] lg:text-6xl">
              {String(live?.currentQuestionNumber ?? 0).padStart(2, '0')}
              <span className="text-2xl text-[#F7F2E7]/25 lg:text-3xl">
          ...
          {state === 'open' && remainingMs !== null && q ? (
            <div className="text-right">
              ...
              <p
                className={`font-mono text-4xl font-bold tabular-nums lg:text-6xl ${
                  remainingMs < 15000 ? 'text-red-400' : 'text-[#F7F2E7]'
                }`}
              >
            ...
            <div className="text-right">
              ...
              <p className="font-mono text-2xl font-bold tracking-tight text-[#C8A951] lg:text-4xl">
      ...
      {/* Countdown rule */}
      {state === 'open' && remainingMs !== null && q && (
        <div className="mb-8 h-1 w-full bg-white/5">
          <div
      ...
      <div className="grid gap-8 lg:grid-cols-12">
        ...
        <div className="lg:col-span-7">
          <AnimatePresence mode="wait">
            {state === 'idle' || !q ? (
              ...
              >
                <Trophy className="mb-8 h-20 w-20 text-[#C8A951]/40" />
                <p className="text-4xl font-bold tracking-tight text-[#F7F2E7] lg:text-5xl">
                  Standing by
                </p>
                <p className="mt-4 font-mono text-lg tabular-nums text-[#F7F2E7]/45">
              ...
              >
                <div className="mb-7 border-l-2 border-[#C8A951] pl-6">
                  <h2 className="text-3xl font-bold leading-tight tracking-tight text-[#F7F2E7] lg:text-5xl">
                    {q.titleEnglish}
                  </h2>
                  <p className="mt-3 text-lg leading-relaxed text-[#F7F2E7]/60 lg:text-2xl">
                    {q.promptEnglish}
                  </p>
                </div>
                ...
                {state !== 'revealed' || !live?.correctOrder ? (
                  <div className="flex flex-wrap gap-2.5">
                    {q.items.map((item) => (
                      <span
                        key={item.key}
                        className="rounded-xl border border-white/12 bg-white/[0.04] px-4 py-2.5 text-lg text-[#F7F2E7]/85 lg:text-2xl"
                      >
                        {item.en}
                      </span>
                    ))}
                  ...
                  <div>
                    <p className="mb-4 font-mono text-xs tracking-[0.3em] text-emerald-300 lg:text-sm">
                      CORRECT SEQUENCE
                    </p>
                      ...
                      {live.correctOrder.map((key, i) => {
                        ...
                        return (
                          <motion.li
                            ...
                            className="flex items-center gap-3 rounded-lg border border-emerald-400/25 bg-emerald-400/[0.07] px-3 py-2"
                          >
                            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-emerald-400 font-mono text-sm font-bold tabular-nums text-[#063B2D]">
                              {i + 1}
                            </span>
                            <span className="truncate text-base text-[#F7F2E7] lg:text-xl">
                              {item?.en ?? key}
                            </span>
                          </motion.li>
                        );
                ...
                <div className="mt-8 flex items-center gap-3 border-t border-white/8 pt-5">
                  <Users className="h-5 w-5 text-[#C8A951]/70" />
                  <span className="font-mono text-2xl font-bold tabular-nums text-[#F7F2E7] lg:text-3xl">
                    {live?.answerCount ?? 0}
                    <span className="text-[#F7F2E7]/25">/{totalPlayers}</span>
                  </span>
                  <span className="font-mono text-xs tracking-[0.2em] text-[#F7F2E7]/35">
                    SUBMITTED
                  </span>
                  <div className="ml-auto h-1.5 w-40 overflow-hidden rounded-full bg-white/8">
                    <motion.div
        ...
        <div className="lg:col-span-5">
          <div className="mb-5 flex items-baseline gap-3 border-b border-[#C8A951]/20 pb-3">
            <Trophy className="h-6 w-6 text-[#C8A951]" />
            <h3 className="text-2xl font-bold tracking-tight text-[#F7F2E7] lg:text-3xl">
              Leaderboard
            </h3>
            <span className="ml-auto font-mono text-xs tracking-[0.2em] text-[#F7F2E7]/35">
              TOP {TOP_N}
            </span>
          </div>

          {board.length === 0 ? (
            <p className="py-16 text-center text-lg text-[#F7F2E7]/30">
                  ...
                  >
                    <span
                      className={`w-9 shrink-0 text-center font-mono text-2xl font-bold tabular-nums lg:text-3xl ${
                        e.rank === 1 ? 'text-[#C8A951]' : 'text-[#F7F2E7]/35'
                      }`}
                    >
                      {e.rank}
                    </span>
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-lg font-semibold text-[#F7F2E7] lg:text-2xl">
                        {e.participantName}
                      </p>
                      <p className="font-mono text-[11px] tabular-nums text-[#F7F2E7]/35 lg:text-xs">
                    ...
                    {e.lastQuestionCorrect !== null && (
                      <span
                        className={`shrink-0 text-lg ${
                          e.lastQuestionCorrect ? 'text-emerald-400' : 'text-red-400/50'
                        }`}
                      >
                    ...
                    <span className="w-10 shrink-0 text-right font-mono text-3xl font-bold tabular-nums text-[#C8A951] lg:text-4xl">
                      {e.score}
                    </span>
                  </motion.div>
                ))}
