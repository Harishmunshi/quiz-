...
export default function Round2Page() {
  ...
  return (
    <Shell>
      {/* Header */}
      <header className="sticky top-0 z-20 border-b border-[#C8A951]/20 bg-[#063B2D]/85 backdrop-blur-md">
        <div className="mx-auto flex max-w-2xl items-center gap-3 px-4 py-3">
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-semibold text-[#F7F2E7]">
              {participant.name}
            </p>
            <p className="truncate text-[11px] text-[#F7F2E7]/45">
              {participant.className}-{participant.division} · {participant.participantCode}
            </p>
          </div>

          {!connected && (
            <span className="flex items-center gap-1.5 rounded-full bg-amber-400/15 px-2.5 py-1 text-[11px] font-semibold text-amber-300">
              <WifiOff className="h-3 w-3" /> Reconnecting
            </span>
          )}

          <span className="shrink-0 rounded-full border border-[#C8A951]/30 px-3 py-1 font-mono text-xs tabular-nums text-[#C8A951]">
        ...
        <AnimatePresence mode="wait">
          ...
          {(state === 'idle' || !q) && (
            <Fade key="idle">
              <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
                ...
                >
                  <Hourglass className="h-10 w-10 text-[#C8A951]" />
                </motion.div>
                <h2 className="text-2xl font-bold tracking-tight text-[#F7F2E7]">
                  You&apos;re in
                </h2>
                <p className="mt-3 max-w-xs text-sm leading-relaxed text-[#F7F2E7]/55">
          ...
          {state === 'open' && q && (
            <Fade key={`open-${q.id}`}>
                ...
                <div className="mb-6 flex items-baseline justify-between">
                  <span
                    className={`font-mono text-4xl font-bold tabular-nums tracking-tight ${
                      remainingMs < 15000 ? 'text-red-400' : 'text-[#C8A951]'
                    }`}
                  >
                  ...
                  <span className="font-mono text-xs tabular-nums text-[#F7F2E7]/40">
              ...
              {/* Submit bar */}
              {!locked ? (
                <div className="sticky bottom-0 z-10 -mx-4 mt-6 border-t border-[#C8A951]/20 bg-[#063B2D]/90 px-4 py-4 backdrop-blur-md">
                  <button
                    onClick={submit}
                    ...
                    className={`flex w-full items-center justify-center gap-2 rounded-xl py-4 text-base font-bold transition-all ${
                      complete
                        ...
                        : 'cursor-not-allowed bg-white/8 text-[#F7F2E7]/35'
                    }`}
                  >
                    {submitting ? (
                  ...
                  <p className="mt-2 text-center text-[11px] text-[#F7F2E7]/35">
                ...
                <motion.div
                  ...
                  className="mt-6 flex items-center justify-center gap-2 rounded-xl border border-[#C8A951]/40 bg-[#C8A951]/10 px-4 py-4 text-sm font-semibold text-[#C8A951]"
                >
                  <Lock className="h-4 w-4" />
            ...
            <Fade key="locked">
              <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
                <motion.div
                  ...
                  className="mb-7 rounded-full border border-[#C8A951]/25 p-6"
                >
                  <Lock className="h-10 w-10 text-[#C8A951]" />
                </motion.div>
                <h2 className="text-2xl font-bold tracking-tight text-[#F7F2E7]">
                  Submissions closed
                </h2>
                <p className="mt-3 text-sm text-[#F7F2E7]/55">
                  {locked
                    ? `Your answer is in — ${formatSeconds(live!.myAnswer!.responseTimeMs)}`
                    : 'You did not submit this one.'}
                </p>
                <p className="mt-1 text-xs text-[#F7F2E7]/35">Waiting for the reveal…</p>
              </div>
            </Fade>
          )}
          ...
          {state === 'revealed' && q && (
            <Fade key={`revealed-${q.id}`}>
              {live?.myAnswer ? (
                ...
                >
                  {live.myAnswer.isCorrect ? (
                    ...
                    <XCircle className="mb-3 h-11 w-11 text-red-400" />
                  )}
                  <p className="text-2xl font-bold tracking-tight text-[#F7F2E7]">
                    {live.myAnswer.isCorrect ? 'Perfect sequence' : 'Not quite'}
                  </p>
                  <p className="mt-2 font-mono text-xs tabular-nums text-[#F7F2E7]/55">
                    {live.myAnswer.correctPositions ?? 0} / {q.itemCount} in place ·{' '}
                    {formatSeconds(live.myAnswer.responseTimeMs)}
                  </p>
                </motion.div>
              ) : (
                <div className="mb-6 rounded-2xl border border-white/12 bg-white/[0.04] p-6 text-center">
                  <p className="text-base font-semibold text-[#F7F2E7]/70">
                    No answer recorded
                  </p>
                </div>
              )}
              ...
              {live?.myAnswer && live.correctOrder && (
                <div className="mb-6">
                  <h3 className="mb-3 text-[11px] font-semibold uppercase tracking-[0.22em] text-[#F7F2E7]/45">
                    Your answer
                  </h3>
                  <SequenceBuilder
                    items={q.items}
              ...
              {live?.correctOrder && (
                <div className="mb-6 rounded-2xl border border-emerald-400/30 bg-emerald-400/[0.06] p-4">
                  <h3 className="mb-3 text-[11px] font-semibold uppercase tracking-[0.22em] text-emerald-300">
                    Correct sequence
                  </h3>
                    ...
                    {live.correctOrder.map((key, i) => {
                      ...
                      return (
                        <li key={key} className="flex items-baseline gap-3">
                          <span className="w-5 shrink-0 text-right font-mono text-[11px] tabular-nums text-emerald-400/70">
                            {i + 1}
                          </span>
                          <span className="text-sm text-[#F7F2E7]">
                            {item?.en ?? key}
                          </span>
                        </li>
                      );
              ...
              {standing && (
                <div className="rounded-2xl border border-[#C8A951]/25 bg-white/[0.04] p-5">
                  <div className="mb-4 flex items-center gap-2">
                    <Trophy className="h-4 w-4 text-[#C8A951]" />
                    <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[#C8A951]">
                      Your position
                    </p>
                  </div>
              ...
              <p className="mt-6 text-center text-xs text-[#F7F2E7]/35">
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <main
      ...
      style={{ background: 'linear-gradient(160deg,#052E23 0%,#063B2D 45%,#0A5E3F 100%)' }}
    >
      {children}
    </main>
  );
...
function QuestionHead({
  ...
  return (
    <div className="mb-6 border-l-2 border-[#C8A951] pl-4">
      <p className="mb-1.5 font-mono text-[11px] uppercase tracking-[0.28em] text-[#C8A951]/70">
        Question {String(q.questionNumber).padStart(2, '0')}
      </p>
      <h2 className="text-xl font-bold leading-snug tracking-tight text-[#F7F2E7]">
        {q.titleEnglish}
      </h2>
      <p className="mt-2 text-sm leading-relaxed text-[#F7F2E7]/65">{q.promptEnglish}</p>
      {q.promptSecondary && (
        <p className="mt-1.5 text-sm leading-relaxed text-[#F7F2E7]/45">
          {q.promptSecondary}
        </p>
      )}
    </div>
...
function Stat({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div>
      <p className="text-[10px] uppercase tracking-wider text-[#F7F2E7]/40">{label}</p>
      <p className="mt-1 font-mono text-lg font-bold tabular-nums text-[#C8A951]">{value}</p>
...
function JoinForm({ onJoined }: { onJoined: (p: StoredParticipant) => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/participant', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, className, division, language }),
      });
      const json = await res.json();
      if (json.success) {
        onJoined({
  ...
  const field =
    'w-full rounded-xl border border-white/12 bg-white/[0.06] px-3.5 py-3 text-[#F7F2E7] outline-none transition-colors placeholder:text-[#F7F2E7]/25 focus:border-[#C8A951]';

  return (
    <div className="flex flex-1 items-center justify-center px-4 py-12">
      <form onSubmit={submit} className="w-full max-w-sm">
        <div className="mb-8 border-l-2 border-[#C8A951] pl-4">
          <p className="font-mono text-[11px] uppercase tracking-[0.28em] text-[#C8A951]/70">
            Round 02
          </p>
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-[#F7F2E7]">
            Take your seat
          </h1>
          <p className="mt-2 text-sm text-[#F7F2E7]/50">
            Enter your details to join the live round.
          </p>
        </div>

        {err && (
          <p className="mb-4 rounded-xl border border-red-400/40 bg-red-500/10 px-3.5 py-2.5 text-sm text-red-200">
            {err}
          </p>
        )}
          ...
          {(['english', 'gujarati'] as const).map((l) => (
            <button
              ...
              className={`rounded-xl border px-3 py-2.5 text-sm font-semibold transition-colors ${
                language === l
                  ? 'border-[#C8A951] bg-[#C8A951]/15 text-[#C8A951]'
                  : 'border-white/12 bg-white/[0.04] text-[#F7F2E7]/60'
              }`}
            >
              {l === 'english' ? 'English' : 'ગુજરાતી'}
            </button>
          ))}
        </div>
        ...
        <button
          type="submit"
          disabled={busy}
          className="flex w-full items-center justify-center rounded-xl bg-[#C8A951] py-4 text-base font-bold text-[#063B2D] transition-colors hover:bg-[#d9bd6b] disabled:opacity-60"
        >
...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <span className="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.18em] text-[#F7F2E7]/45">
      {children}
    </span>
  );
}
